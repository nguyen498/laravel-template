...
class EmployeeService extends BaseService
{
    ...
    public function login($inputs) {
        ...
        if(isset($employ)) {
            if($employ->status === Employee::STATUS_UNACTIVE){
                ...
                Auth::logout();
                Auth::login($employ);
                $token = $employ->createToken(config('constants.default_app'), ['employees'])->accessToken;
    ...
    public function loginWithPassword($inputs){
        ...
        $token = $employee->createToken(config('constants.default_app'), ['employees'])->accessToken;
        return [
            'code' => '200',
            'data' => [
    ...
    public function getPermission() {
        $employ = Auth::guard('employees')->user();
        return [
            'code' => '200',
